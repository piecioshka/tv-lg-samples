...
//						'<span id="', IDs.size ,'" title="contract" op="resize"></span>',	// by Sungsik Kim
						'<span class="clear" title="clear" op="clear"></span>',
//						'<span class="close" title="close" op="close"></span>',				// by Sungsik Kim
					'</div>',
				'</div>',
			'</div>',
			'<div class="main">',
			...
			'<div class="footer">',
				'<div class="left">',
					'<label for="bbDes">',
						'<div style="float:left"><span class="lgobject" type="lgobject"></span>object</div>',
						'<div style="float:left"><span class="lgmethod" type="lgmethod"></span>method</div>',
						'<div style="float:left"><span class="lgproperty" type="lgproperty"></span>property</div>',
						'<div style="float:left"><span class="lgevent" type="lgevent"></span>event</div>',
					'</label>',
				'</div>',
				'<div class="right"></div>',
			'</div>'
	...
	function addMessage( type, content ) { //adds a message to the output list
		...
		if ( outputList ) {
			var logs=outputList.getElementsByTagName('li');
			var count=logs.length;
			if(count > 50) {
				while(count--){
					outputList.removeChild(logs[length]);
				}
			}
	...
	function reposition( position ) {
		if ( position === undefined || position == null ) {
			position = ( state && state.pos === null ) ? 1 : ( state.pos + 1 ) % 5; //set to initial position ('topRight') or move to next position
		}
		...
		switch ( position ) {
			...
			case 4: classes[ 0 ] = 'bbMiddleRight'; break;
		}
		state.pos = position;
		setState();
	}
...
//				resize(1);
		...
		move:
			function() { reposition(); },
		lgobject: 
			function( msg ) { addMessage( 'lgobject', msg ); },
		lgproperty:
			function( msg ) { addMessage( 'lgproperty', msg ); },
		lgmethod:
			function( msg ) { addMessage( 'lgmethod', msg ); },
		lgevent:
			function( msg ) { addMessage( 'lgevent', msg ); },
		profile: 
			function( label ) {
